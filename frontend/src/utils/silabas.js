...
export default (function () {
  ...
  let encontroTonica // (bool)   Indica si se ha encontrado la sílaba tónica
  ...
  function posicionSilabas(palabra) {
    ...
    for (let actPos = 0; actPos < silaba.longitudPalabra;) {
      ...
      if ((encontroTonica) && (silaba.tonica == 0)) { silaba.tonica = silaba.numeroSilaba } // Marca la sílaba tónica
    }
  ...
  function ataque(pal, pos) {
    ...
    if (pos < silaba.longitudPalabra - 1) {
      if (pal[pos] == 'u') {
        if (ultimaConsonante == 'q') { pos++ } else if (ultimaConsonante == 'g') {
  ...
  /**
     ...
     */
  function hiato() {
    let vocalFuerteAnterior = false // Almacena el tipo de vocal (Fuerte o Débil)
    silaba.hiato = []
